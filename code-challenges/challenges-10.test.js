...
const createServer = () => {
  ...
  app.get('/aboutme', function (request, response) {
    response.send('Hello my name is Terrence and I am 27 years old, I am a full stack software developer.');
  });
  ...
  function foodHandler(request, response) {
    let foodArray = ['pizza', 'shrimp', 'pears', 'chicken'];
...
const count = (target, input) => {
  let count = 0;
  input.filter(outerArr => {
    outerArr.filter(inArray => {
      if (inArray === target) {
        count++;
      }
    });
...
const totalSum = (input) => {
  const mainArray = input.reduce((acc, value) => {
    value.forEach(value => {
      acc += value;
    });
    return acc;
