...
const countNumberOfElements = (arr) => {
  const newArray = arr.reduce(total => {
    return total + 1;
  }, 0);
  return newArray;
};
...
const returnNames = (arr) => {
  let newArray = arr.reduce((acc, value, idx) => {
    acc[idx] = value.name;
    return acc;
  }, []);
  return newArray;
...
const reversedString = (str) => {
  ...
  let holder = stringArray.reduce((acc, value) => {
    acc.unshift(value);
    return acc;
  }, []);
  return holder.join('');
...
const countNumberOfChildren = (arr) => {
  const mainArray = arr.reduce((acc, value) => {
    let kid = value.children;
    if (!kid) {
      //do nothing
    } else {
      acc += kid.length;
...
const calculateAverage = (arr) => {
  const mainArray = arr.reduce((acc, value, idx) => {
    acc += value;
    if (idx === arr.length - 1) {
      acc /= arr.length;
    }
...
const extractChildren = (arr) => {
  ...
  const reduceChildren= filterArr.reduce((acc,val) =>{
    if(val.children){
